...
CONFIG = {
    ...
    'balance': 1000,
    'leverage': 20,
    'fixed_lot': 100,              # ФИКСИРОВАННЫЙ ОБЪЕМ В USDT (с учетом плеча)
    'stop_loss_pct': 0.012,        # 1.2%
    'take_profit_pct': 0.035,       # 3.5%
    'breakeven_trigger': 0.01,     # БУ при +1%
    'trailing_distance': 0.008,    # Трейлинг 0.8%
    'commission_rate': 0.0011,     # Комиссия (вход + выход)
}
...
class TradeJournal:
    ...
    def log_trade(self, symbol, side, result, entry, exit_p, start_time):
        try:
            ...
            # Расчет прибыли от ФИКСИРОВАННОГО лота
            position_size_usdt = CONFIG['fixed_lot']
...
def add_indicators(df, cfg):
    ...
    df['adx'] = (abs(plus_dm.rolling(cfg['adx_period']).mean() - minus_dm.rolling(cfg['adx_period']).mean()) / 
                (plus_dm.rolling(cfg['adx_period']).mean() + minus_dm.rolling(cfg['adx_period']).mean()) * 100).rolling(cfg['adx_period']).mean()
...
def get_signal(df):
    ...
    c = df.iloc[-1]
    if c['adx'] < 20: return None # Фильтр боковика

    if (c['ema_fast'] > c['ema_mid'] > c['ema_slow'] and c['close'] > c['ema_fast'] and 
        45 < c['rsi'] < 65 and c['macd_line'] > c['macd_signal'] and c['volume'] > c['vol_ma'] * 0.9):
        return 'LONG'

    if (c['ema_fast'] < c['ema_mid'] < c['ema_slow'] and c['close'] < c['ema_fast'] and 
        35 < c['rsi'] < 55 and c['macd_line'] < c['macd_signal'] and c['volume'] > c['vol_ma'] * 0.9):
        return 'SHORT'
    return None
...
class SignalBot:
    ...
    async def scan(self, app_bot):
        for trade in self.active_trades[:]:
            try:
                ...
                if is_sl or is_tp:
                    res = 'PROFIT' if is_tp else 'STOP/TRAILING'
    ...
    async def _open_trade(self, app_bot, symbol, side, price):
        ...
        # ИСПОЛЬЗУЕМ ФИКСИРОВАННЫЙ ЛОТ
        size = self.cfg['fixed_lot']
...
async def active_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for t in bot_instance.active_trades:
        msg += f"• {t['symbol']} ({t['side']}) | Вход: {t['entry']}\n"
