...
CONFIG = {
    ...
    'fixed_order_size': 1000,       # УСТАНОВЛЕНО 1000
    'leverage': 20,
    'risk_per_trade': 0.02,
    'stop_loss_pct': 0.015,
    ...
    'cooldown_minutes': 30,         # ТВОЙ КУЛДАУН
}
...
async def send_daily_report(context: ContextTypes.DEFAULT_TYPE):
    ...
    if df_today.empty:
        await context.bot.send_message(chat_id=CONFIG['chat_id'], text="🌙 <b>Итоги дня:</b> сделок не было.", parse_mode='HTML')
        return
    total_pnl = round(df_today['profit_usdt'].sum(), 2)
    wins = len(df_today[df_today['profit_usdt'] > 0])
    wr = round((wins / len(df_today)) * 100, 1)
    msg = (f"📅 <b>ИТОГИ ДНЯ (Киев)</b>\n━━━━━━━━━━━━━━━\n"
           f"💰 PnL: <b>{total_pnl}$</b>\n🎯 Win Rate: <b>{wr}%</b>\n"
...
class TradeJournal:
    ...
    def log_trade(self, symbol, side, result, entry, exit_p, start_time):
        try:
            ...
            comm = pos_size * CONFIG['commission_rate']
            profit_usdt = (pos_size * price_diff_pct) - comm
            now = datetime.now()
            new_row = {
...
class SignalBot:
            ...
            try:
                ...
                if is_sl or is_tp or exit_reason:
                    ...
                    if data:
                        ...
                        await app_bot.send_message(chat_id=self.cfg['chat_id'], text=f"{icon} <b>ЗАКРЫТО: {trade['symbol']}</b>\nPnL: {data['profit_usdt']}$", parse_mode='HTML')
                    self.active_trades.remove(trade)
            except Exception as e: logger.error(f"Scan error: {e}")

        for symbol in self.cfg['symbols']:
            if any(t['symbol'] == symbol for t in self.active_trades): continue
            if symbol in self.last_exit_time and (time.time() - self.last_exit_time[symbol] < self.cfg['cooldown_minutes'] * 60): continue
            try:
    ...
    async self._open_trade(self, app_bot, symbol, side, price):
        ...
        self.active_trades.append({'symbol': symbol, 'side': side, 'entry': price, 'sl': sl, 'tp': tp, 'size_usdt': 1000, 'start_time': datetime.now()})
        await app_bot.send_message(chat_id=self.cfg['chat_id'], text=f"💎 <b>ВХОД {symbol}</b> ({side})\nЦена: {price}\nОбъем: 1000 USDT", parse_mode='HTML')

# (Команды /start, /stats и т.д. остаются из предыдущего рабочего кода)
...
async def main():
    ...
    async with app:
        ...
        while True:
            await bot_instance.scan(app.bot); await asyncio.sleep(30)
