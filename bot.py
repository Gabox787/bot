    ...
    async def scan(self, app_bot):
        ...
        for trade in self.active_trades[:]:
            try:
                ...
                curr_p = ticker['last']
                profit_now = ((curr_p - trade['entry']) / trade['entry']) if trade['side'] == 'LONG' else (
                    (trade['entry'] - curr_p) / trade['entry'])

                # Trailing Stop
                if profit_now >= self.cfg['breakeven_trigger']:
                    if trade['side'] == 'LONG':
                        new_sl = round(curr_p * (1 - self.cfg['trailing_distance']), 8)
                        if new_sl > trade['sl']:
                            trade['sl'] = new_sl
                    else:
                        new_sl = round(curr_p * (1 + self.cfg['trailing_distance']), 8)
        ...
        for symbol in self.cfg['symbols']:
            ...
            try:
                raw = await asyncio.to_thread(
                    self.exchange.fetch_ohlcv, symbol, self.cfg['timeframe'], limit=50
                )
                df = add_indicators(
                ...
                if self.last_candle.get(symbol) == str(c['ts']):
                    continue
