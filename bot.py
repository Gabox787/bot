...
CONFIG = {
    ...
    'timeframe': '15m',  # ИЗМЕНЕНО: теперь 15 минут для более точного входа
    'ema_fast': 9,
    'ema_mid': 21,
    'ema_slow': 50,
    'rsi_period': 14,
...
async def active_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for t in bot_instance.active_trades:
        try:
            ...
            # --- ИСПРАВЛЕННЫЙ БЛОК СО СМАЙЛИКАМИ ---
            icon = "🟢" if pnl_usdt > 0 else ("🔴" if pnl_usdt < 0 else "⚪️")
